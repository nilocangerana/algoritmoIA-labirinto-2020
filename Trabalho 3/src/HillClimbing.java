...
public class HillClimbing {
	...
	ArrayList<Integer> noAtual = new ArrayList<>();
	...
	public int menorManhattanDistance()
	{
		...
		int minimoIndex=0;
		int mDistance;
		int flagPossuiVizinho=0;
		
		for(int i=0;i<fila.size();i++)
		{
			mDistance=calculaManhattanDistance(fila.get(i).get(0),fila.get(i).get(1),posicaoFinal[0],posicaoFinal[1]);
			if(mDistance<=minimo && fila.get(i).get(2)==noAtual.get(0) && fila.get(i).get(3)==noAtual.get(1))
			{
				minimo=mDistance;
				minimoIndex=i;
				flagPossuiVizinho++;
	...
	public void buscaHC(char m[][], int[] posInicio)
	{
		int flag=1;
		int contador=0;
		...
		while(flag==1)
		{
			...
			noAtual.add(fila.get(0).get(0));
			noAtual.add(fila.get(0).get(1));
			noAtual.add(fila.get(0).get(2));
			noAtual.add(fila.get(0).get(3));
			contador++;
			fila.remove(0);
		}
	}
